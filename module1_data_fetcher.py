...
_EVENT_KEYWORDS: dict[EventType, list[str]] = {
    ...
    EventType.ANALYST: [
        "analyst", "upgrade", "downgrade", "price target", "rating", "coverage",
        "overweight", "underweight", "outperform", "underperform",
        "initiates coverage", "raises target", "cuts target",
    ],
    EventType.REGULATORY: [
        "sec", "regulation", "fda", "antitrust", "lawsuit", "compliance", "fine",
        "export control", "export ban", "chip ban", "sanction", "chips act",
    ],
    EventType.MACRO: [
        "fed", "interest rate", "inflation", "recession", "gdp", "unemployment", "tariff",
        "trade war", "rate hike", "rate cut", "treasury yield", "sell-off",
    ],
    EventType.PRODUCT: [
    ...
    EventType.PERSONNEL: [
        "ceo", "cfo", "coo", "cto",
        "appointed", "resigns", "resignation", "steps down", "named as",
        "new chief", "new president",
    ],
}

# Titles matching these patterns are market-summary noise — skip at parse time.
...
class FinnhubNewsFetcher(NewsFetcher):
    ...
    def _parse_article(article: dict, fallback_date: date) -> MarketEvent | None:
        ...
        if any(p in hl_lower for p in _NOISE_TITLE_PATTERNS):
            return None

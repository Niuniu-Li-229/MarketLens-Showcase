...
Module 3 — Sentiment Analysis + Price Forecasting
        ...
        └── FinBERTAnalyzer          ← real implementation

        PriceForecaster (abstract)
        ├── MockForecaster           ← heuristic mock
        ├── LSTMForecaster           ← baseline model
        └── TransformerForecaster    ← improved model

        ForecastResult               ← data class returned by both models
"""
...
class ForecastResult:
    """
    Returned by both LSTMForecaster and TransformerForecaster.
    Contains everything Module 5 needs to draw the prediction chart.
    """
    model_name:   str
    day5_price:   float                    # Day 5 predicted price (→ AnalysisResult)
    forecast_5d:  list[float]              # [Day1, Day2, Day3, Day4, Day5]
...
# ── Sector mapping (shared by both forecasters) ───────────────────────────────
...
class FinBERTAnalyzer(SentimentAnalyzer):
    """
    Real sentiment using ProsusAI/finbert.
    ...
    def analyze(self, events: list[MarketEvent]) -> tuple[float, str]:
        ...
        print(f"[Module 3] Sentiment: {label} ({avg:+.3f}) across {len(events)} events.")
...
def _build_base_features(
    ...
    """2 features: close + anomaly flag. Used by baseline LSTM."""
...
def _make_sequences(features: np.ndarray, seq_len: int, split: float = 0.85):
    """Build sliding-window sequences and split into train/test."""
...
def _inverse_close(arr: np.ndarray, scaler, n_features: int) -> np.ndarray:
    pad = np.zeros((len(arr), n_features))
    pad[:, 0] = arr
    return scaler.inverse_transform(pad)[:, 0]


def _roll_forecast(model, scaled: np.ndarray, seq_len: int,
                   scaler, n: int, days: int = 5) -> list[float]:
    """Roll the model forward N days autoregressively."""
    import torch
    ...
    for _ in range(days):
        ...
        with torch.no_grad():
            out = model(inp).item()
        pad = np.zeros((1, n)); pad[0, 0] = out
        prices.append(round(float(scaler.inverse_transform(pad)[0, 0]), 2))
        new_row    = seq[-1].copy(); new_row[0] = out
        seq        = np.vstack([seq[1:], new_row])
    return prices


def _print_forecast(prices: list, last_close: float,
                    last_date, model_name: str):
...
class LSTMForecaster(PriceForecaster):
    """
    Baseline model — 2 features, seq=10, 1-layer LSTM.
    Fast to train. Used as the comparison baseline in poster Chart 3.
    Returns Day 5 predicted price.
    """
    SEQ_LEN = 10
    HIDDEN  = 32
    EPOCHS  = 80
...
class TransformerForecaster(PriceForecaster):
    """
    Improved model — 8 features (incl. RSI, MACD, Bollinger, sector),
    seq=20, 2-layer Transformer encoder with attention mechanism.
